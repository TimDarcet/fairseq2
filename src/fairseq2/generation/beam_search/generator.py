...
class BeamSearchSequenceGenerator(SequenceGenerator):
    ...
    def __init__(
        ...
        algo: BeamSearchAlgorithm | None = None,
        *,
        beam_size: int = 5,
        min_gen_len: int = 1,
        max_gen_len: int = 128,
        ...
        self._algo = algo or StandardBeamSearchAlgorithm()
        ...
        self._step_hooks: dict[int, StepHook] = OrderedDict()
...
class BeamSearchSeq2SeqGenerator(Seq2SeqGenerator):
    ...
    def __init__(
        ...
        *,
        beam_size: int = 5,
        min_gen_len: int = 1,
        max_gen_len: tuple[float, int] = (1.0, 128),
        max_seq_len: int | None = None,
        echo_prompt: bool = False,
        normalize_scores: bool = True,
        temperature: float = 1.0,
...
class _AbstractBeamSearchSequenceGeneratorOp(ABC):
    def __init__(
        ...
        if self._max_prompt_len >= max_seq_len:
            raise ValueError(
                f"Maximum prompt length in `prompt_seqs` must be less than `max_seq_len` ({max_seq_len}), but is {self._max_prompt_len} instead."
            )
        ...
        self._prompt_lens: Tensor | None
        self._prompt_mask: Tensor | None
        ...
        self._output: list[list[Hypothesis]] = [[] for _ in range(num_prompts)]
    ...
    def _step(self) -> bool:
        ...
        if lprobs.isnan().any():
            raise SequenceGenerationError(self._step_nr)
    ...
    def _search_beam(
        ...
        if self._step_nr < self._max_prompt_len:
            ...
            if self._prompt_mask[batch_offset, self._step_nr]:
                ...
                if len(lprobs) != 1:
                    raise InternalError(f"Length of `lprobs` is {len(lprobs)}.")
