...
class TrainUnit(ABC, Generic[BatchT_contra]):
    ...
    def process_batch(
        ...
        """
        Returns the loss and the number of targets used to compute the loss. If
        the number of targets is ``None``, the trainer won't normalize model
        gradients.
        """
...
class Trainer(Task):
    ...
    def _maybe_request_early_stop(self, score: float) -> bool:
        if self._early_stopper is NOOP_EARLY_STOPPER:
            return False
